  ...
  return (
    <div className="flex flex-col gap-[20px] w-full">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-[14px]">
        <div>
          <p className="font-['Wix_Madefor_Display'] font-bold text-[24px] md:text-[28px] text-[#1185e6]">Пользователи</p>
          <p className="font-['Wix_Madefor_Display'] text-[14px] md:text-[15px] text-[#8a92a3]">Управление учётными записями системы</p>
        </div>
        <UsMotion.button whileTap={{ scale: 0.96 }}
          onClick={() => setOpen(true)}
          className="bg-[#1185e6] hover:bg-[#0d6fc0] transition-colors text-white font-['Wix_Madefor_Display'] font-medium text-[14px] md:text-[15px] px-[18px] md:px-[20px] py-[10px] md:py-[12px] rounded-[16px] md:rounded-[18px] flex items-center gap-[10px] shadow-[0_6px_20px_rgba(17,133,230,0.30)] w-fit">
      ...
      <div className="bg-[#1c2333] rounded-[28px] overflow-hidden shadow-[0_8px_28px_rgba(0,0,0,0.30)] border border-white/5">
        <div className="hidden md:grid grid-cols-[2fr_3fr_1.4fr_1.4fr_60px] px-[28px] py-[16px] bg-[#252b3d] border-b border-white/5">
          <span className="font-['Wix_Madefor_Display'] font-medium text-[13px] text-[#8a92a3] uppercase tracking-wide">Логин</span>
          <span className="font-['Wix_Madefor_Display'] font-medium text-[13px] text-[#8a92a3] uppercase tracking-wide">Имя</span>
          <span className="font-['Wix_Madefor_Display'] font-medium text-[13px] text-[#8a92a3] uppercase tracking-wide">Роль</span>
          <span className="font-['Wix_Madefor_Display'] font-medium text-[13px] text-[#8a92a3] uppercase tracking-wide">Последний вход</span>
          <span></span>
        </div>
          ...
          {users.map(u => (
            <div key={u.id} className="grid grid-cols-[1fr_auto] md:grid-cols-[2fr_3fr_1.4fr_1.4fr_60px] gap-y-[6px] gap-x-[12px] items-center px-[16px] md:px-[28px] py-[14px] md:py-[16px] border-b border-white/5 hover:bg-white/[0.03] transition-colors">
              <div className="flex items-center gap-[12px]">
                <div className="size-[34px] rounded-full bg-[#1185e6] text-white flex items-center justify-center font-['Wix_Madefor_Display'] font-bold text-[13px]">{u.login.slice(0, 2).toUpperCase()}</div>
                <div className="flex flex-col md:flex-row md:items-center md:gap-[12px] min-w-0">
                  ...
                  <span className="md:hidden font-['Wix_Madefor_Display'] text-[13px] text-[#8a92a3] truncate">{u.name}</span>
                </div>
              </div>
              ...
              <span className="hidden md:inline font-['Wix_Madefor_Display'] text-[13px] text-[#8a92a3]">{u.last}</span>
              <div className="flex items-center gap-[10px] justify-end">
                ...
                <button className="text-[#8a92a3] hover:text-[#ff4d5e] transition-colors" onClick={() => setUsers(p => p.filter(x => x.id !== u.id))}>
        ...
        {open && (
          <UsMotion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-[3px]"
            ...
            <UsMotion.form onSubmit={submit} onClick={(e) => e.stopPropagation()}
              ...
              className="bg-[#252b3d] rounded-[24px] p-[20px] md:p-[28px] w-[calc(100vw-32px)] max-w-[460px] shadow-[0_24px_64px_rgba(0,0,0,0.6)] border border-white/8">
              ...
              <div className="flex flex-col gap-[14px]">
                {[
                  ...
                  <label key={f.k} className="flex flex-col gap-[6px]">
                    <span className="font-['Wix_Madefor_Display'] text-[12px] uppercase tracking-wide text-[#8a92a3]">{f.l}</span>
                    <input type={f.t} value={form[f.k]} onChange={e => setForm({ ...form, [f.k]: e.target.value })}
                      className="bg-[#1c2333] border border-white/8 rounded-[14px] px-[16px] py-[12px] font-['Wix_Madefor_Display'] text-[15px] text-white focus:outline-none focus:ring-2 focus:ring-[#1185e6] focus:border-transparent" />
                  </label>
                ))}
                <label className="flex flex-col gap-[8px]">
                  <span className="font-['Wix_Madefor_Display'] text-[12px] uppercase tracking-wide text-[#8a92a3]">Роль</span>
                  <div className="flex gap-[8px]">
                    {ROLES.map(r => (
                      <button type="button" key={r} onClick={() => setForm({ ...form, role: r })}
                        className={`flex-1 py-[10px] rounded-[12px] font-['Wix_Madefor_Display'] font-medium text-[14px] transition-colors ${form.role === r ? "bg-[#1185e6] text-white" : "bg-[#1c2333] text-[#8a92a3] hover:text-white"}`}>
                        {r}
                      </button>
                    ))}
                  </div>
              ...
              <div className="flex gap-[10px] mt-[22px] justify-end">
                <button type="button" onClick={() => setOpen(false)} className="px-[18px] py-[10px] rounded-[14px] font-['Wix_Madefor_Display'] text-[14px] text-[#8a92a3] hover:bg-[#1c2333]">Отмена</button>
                <button type="submit" className="bg-[#1185e6] hover:bg-[#0d6fc0] transition-colors px-[20px] py-[10px] rounded-[14px] font-['Wix_Madefor_Display'] font-bold text-[14px] text-white">Создать</button>
              </div>
            </UsMotion.form>
          </UsMotion.div>
        )}
